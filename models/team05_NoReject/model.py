...
class DRRE(nn.Module):
    def __init__(self, args):
        ...
        self.residual_enhance_blocks = args.residual_enhance_blocks
        if self.residual_enhance_blocks > 0:
            self.residual_enhance_layer = [nn.Conv2d(8, args.residual_enhance_block_dim, 1)] + \
                                          [RRDB(num_feat=args.residual_enhance_block_dim) for _ in range(self.residual_enhance_blocks)] + \
                                          [nn.Conv2d(args.residual_enhance_block_dim, 4, 1)]
            self.residual_enhance_layer = nn.Sequential(*self.residual_enhance_layer)
    ...
    def _move_models_to_device_and_dtype(self):
        ...
        if self.residual_enhance_blocks > 0:
            self.residual_enhance_layer.to(self.device)
            self.residual_enhance_layer.requires_grad_(False)
    ...
    def _load_and_save_ckpt_from_state_dict(self, sd):
        ...
        if self.residual_enhance_blocks > 0:
            for name, param in self.residual_enhance_layer.named_parameters():
                param.data.copy_(sd["state_dict_residual_enhancer"][name])
                print(f"Load param {name}")
    ...
    def forward(self, default, c_t, prompt=None):
        ...
        if self.residual_enhance_blocks > 0:
            model_pred = model_pred + self.residual_enhance_layer(torch.cat([encoded_control, model_pred], dim=1))
